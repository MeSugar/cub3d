...
/*   Updated: 2021/03/27 16:52:21 by gdelta           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		image_init(t_win *window_config)
{
	...
	if (!(window_config->image->addr =
	mlx_get_data_addr(window_config->image->img_ptr,
	&window_config->image->bpp, &window_config->image->line_l,
	&window_config->image->endian)))
	...
	if (!(window_config->ray = ft_calloc(1, sizeof(t_ray))))
		return (put_error_msg("Error: Malloc error\n"));
	if (!(window_config->buff = ft_calloc(window_config->win_w,
	sizeof(double))))
		return (put_error_msg("Error: Malloc error\n"));
	return (1);
}
