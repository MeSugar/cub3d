...
int draw_screen(t_win *window_config)
{
    ...
    if (!(window_config->buf = ft_calloc(window_config->window_height, sizeof(int *))))
        return (put_error_msg("Error: Malloc error\n"));
    i = 0;
    while (i < window_config->window_height)
    {
        if (!(window_config->buf[i] = ft_calloc(window_config->window_width, sizeof(int))))
            return (put_error_msg("Error: Malloc error\n"));
        i++;
    }
    ...
    draw_image(window_config, rays);
    draw_map(window_config, window_config->buf);
