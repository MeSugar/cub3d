...
{
	mlx_hook(window_config->win_ptr, 2, 1L << 0,
...
static int	init(t_win *window_config)
{
	if (!map_player_buttons_sprite_init(window_config)
...
int			main(int ac, char **av)
{
	...
	if (ac == 2 && name_check(av[1], ".cub"))
	{
		if (!(window_config = window_config_init(av[1], 0)))
			return (finish_program(window_config, 1));
		if (!init(window_config))
			return (finish_program(window_config, 1));
		loop(window_config);
	}
	else if (ac == 3 && name_check(av[1], ".cub")
	&& save_flag_check(av[2], "--save"))
	{
		...
		return (put_error_msg("Error: Invalid arguments\n"));
}
